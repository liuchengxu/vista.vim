...
let s:plugin_root_dir = fnamemodify(resolve(expand('<sfile>:p')), ':h:h:h')
execute 'set rtp^='.s:plugin_root_dir
...
function! s:CatchException(catch_point)
  call add(v:errors, a:catch_point.':')
  call add(v:errors, '  File: '.g:testpath)
  call add(v:errors, '  Throwpoint: '.v:throwpoint)
...
func RunTest(test)
  ...
  if exists('*SetUp_' . a:test)
    try
      ...
      call s:CatchException('Caught exception in SetUp_' . a:test . ' before '.a:test)
    endtry
  endif
  ...
  if exists('*SetUp')
    try
      ...
      call s:CatchException('Caught exception in SetUp() before ' . a:test)
    endtry
  endif
  ...
  try
    ...
    call s:CatchException('Caught exception in ' . a:test)
    call s:TestFailed()
  endtry
  ...
  if exists('*TearDown')
    try
      ...
      call s:CatchException('Caught exception in TearDown() after ' . a:test)
    endtry
  endif
  ...
  if exists('*TearDown_' . a:test)
    try
      ...
      call s:CatchException('Caught exception in TearDown_' . a:test . ' after ' . a:test)
    endtry
  endif
...
redir => s:TestFunctions
silent function /^Test_
redir END
let s:tests = split(substitute(s:TestFunctions, 'function \(\k*()\)', '\1', 'g'))
